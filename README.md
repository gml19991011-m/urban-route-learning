...
How can repeated travel-time feedback lead to stable route preferences?

How does an agent adapt when a previously preferred route becomes substantially worse?

How does learning rate influence the speed and stability of adaptation following environmental change?
...
Run the likelihood-based parameter-recovery demonstration:
...
The parameter-recovery demonstration also uses a fixed random seed to generate the synthetic behavioural dataset, allowing the fitting result to be reproduced.
...
fitting to empirical human behavioural data.
...
The parameter-recovery analysis is based on a single synthetic participant. It demonstrates the basic logic of likelihood-based parameter estimation but does not establish parameter identifiability or recovery accuracy across heterogeneous populations.

Possible Extensions

Future extensions could include:

fitting the model to empirical human route-choice data,

parameter recovery across larger populations of simulated participants,

systematic evaluation of parameter identifiability,
...
A natural next step would be to apply the same inference framework to empirical behavioural data:

Observed human choices and outcomes
                ↓
       Computational model
                ↓
...
This would move the project from synthetic proof-of-concept modelling toward the estimation of latent learning processes from observed human behaviour.
